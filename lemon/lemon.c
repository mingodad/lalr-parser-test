...
struct symbol {
  ...
  int id;                  /* Unique sequential id for this symbol */
...
struct lemon {
  ...
  int lastSymId;            /* Sequential id for symbols */
  ...
  int nconflict_sr;        /* Number of shift/reduce parsing conflicts */
  int nconflict_rr;        /* Number of reduce/reduce parsing conflicts */
  ...
  int nDefineUsed;    /* Number of -D options actually used */
  char **azDefine;  /* Name of the -D macros */
  char *bDefineUsed;  /* True for every -D macro actually used */
  char *outputDir;
...
static int resolve_conflict(struct lemon *lemp, struct action *,struct action *);
    ...
    for(ap=stp->ap; ap && ap->next; ap=ap->next){
      for(nap=ap->next; nap && nap->sp==ap->sp; nap=nap->next){
         ...
         ** Figure out which one should be used */
         lemp->nconflict += resolve_conflict(lemp, ap,nap);
      }
    }
  }
...
static void handle_D_option(struct lemon *lem, char *z){
  ...
  lem->bDefineUsed = (char*)realloc(lem->bDefineUsed, lem->nDefine);
  if( lem->bDefineUsed==0 ){
    fprintf(stderr,"out of memory\n");
    exit(1);
  }
  lem->bDefineUsed[lem->nDefine-1] = 0;
...
int main(int argc, char **argv){
  ...
  */
...

  {
    ...
    if( !rpyflag2 ) FindRulePrecedences(&lem);
  ...
  if( statistics ){
    ...
    if(lem.nconflict) {
        stats_line("conflicts S/R", lem.nconflict_sr);
        stats_line("conflicts R/R", lem.nconflict_rr);
    }
...
void Reprint_token_numbers(struct lemon *lemp)
{
  struct symbol *sp;
...
void Reprint_tokens(struct lemon *lemp, const char *terminator, int isYacc)
{
  ...
  for(j=0; j<=lemp->preccounter; ++j){
    ...
    for(i=0; i<lemp->nsymbol; i++){
      sp = lemp->symbols[i];
      if(sp->type == TERMINAL && sp->prec == j) {
          if(!skip) {
              skip = 1;
              const char *sassoc = "UNKNOWN";
              switch(sp->assoc) {
                  case LEFT:
                      sassoc = "left";
                      break;
...
/*
** Compare to symbols structures for sorting by line purposes
*/
static int ruleline_compare(const void *a, const void *b){
  const struct rule *p1 = *(const struct rule **)a;
  const struct rule *p2 = *(const struct rule **)b;
...
void Reprint(struct lemon *lemp)
{
  ...
  printf("\n");
#if 0
  array = (struct rule **)calloc(lemp->nrule, sizeof(struct rule *));
  idxRule = 0;
  for(rp=lemp->rule; rp; rp=rp->next){
      array[idxRule++] = rp;
  }
  qsort(array, lemp->nrule, sizeof(struct rule *), ruleline_compare);
  for(i=0; i<lemp->nrule; i++){
...
/* Print the text of a rule
*/
void rule_print_yacc0(FILE *out, struct rule *rp, int mt_idx){
  int i, j;
  fprintf(out, "%s", rp->lhs->name);
  /*    if( rp->lhsalias ) fprintf(out,"(%s)",rp->lhsalias); */
  fprintf(out," :");
...
void rule_print_yacc(FILE *out, struct rule *rp){
    int i, j;
    for(i=0; i<rp->nrhs; i++){
      struct symbol *sp = rp->rhs[i];
      if( sp->type==MULTITERMINAL ){
...
/* Duplicate the input file without comments and without actions
** on rules */
void Reprint_yacc(struct lemon *lemp)
{
  struct rule *rp, *prev_rp;
  struct symbol *sp;
  /*struct rule **array;*/
...
PRIVATE FILE *tplt_open(struct lemon *lemp)
{
  ...
  if( access(buf,004)==0 ){
    ...
    toFree = tpltname = pathsearch(lemp->argv[0],lemp->templatename,0);
  }
  if( tpltname==0 ){
    fprintf(stderr,"Can't find the parser driver template file \"%s\".\n",
    lemp->templatename);
...
void ReportSQL(
  ...
  if(lemp->stacksize) {
    fprintf(sql,
      "UPDATE directives SET stack_size='%s';\n",
          lemp->stacksize
    );
  }
...
void ReportTable(
  ...
  if( lemp->nDefineUsed==0 ){
    fprintf(out, ".\n*/\n"); lineno += 2;
  }else{
    fprintf(out, " with these options:\n**\n"); lineno += 2;
    for(i=0; i<lemp->nDefine; i++){
      if( !lemp->bDefineUsed[i] ) continue;
      fprintf(out, "**   -D%s\n", lemp->azDefine[i]); lineno++;
    }
    fprintf(out, "*/\n"); lineno++;
  }
